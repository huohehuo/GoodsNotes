...
public class BaseDataColorRyAdapter extends RecyclerArrayAdapter<ColorBean> {
    Context context;
    public BaseDataColorRyAdapter(Context context) {
        super(context);
    }
...
//        }
    ...
    class MarkHolder extends BaseViewHolder<ColorBean> {
        ...
        @Override
        public void setData(ColorBean data) {
            super.setData(data);
            name.setText(data.FName);
            if (null!=client){
                if (beanDao.queryBuilder().where(BuyAtBeanDao.Properties.FBuyName.eq(client),BuyAtBeanDao.Properties.FColorName.eq(data.FName)).count()>0){
                    time.setText("已添加");
                }else{
                    time.setText("");
                }
