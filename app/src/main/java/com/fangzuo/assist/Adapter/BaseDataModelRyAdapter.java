...
public class BaseDataModelRyAdapter extends RecyclerArrayAdapter<ModelBean> {
    Context context;
    public BaseDataModelRyAdapter(Context context) {
        super(context);
    }
...
//        }
    ...
    class MarkHolder extends BaseViewHolder<ModelBean> {
        ...
        public void setData(ModelBean data) {
            ...
            if (null!=client){
                if (beanDao.queryBuilder().where(BuyAtBeanDao.Properties.FBuyName.eq(client),BuyAtBeanDao.Properties.FModelName.eq(data.FName)).count()>0){
                    time.setText("已添加");
                }else{
                    time.setText("");
                }
