...
public class ColorBeanFragment extends BaseFragment {
    ...
    Button btnAdd;
    BaseDataColorRyAdapter adapter;
    private FragmentActivity mContext;
    private ColorBeanDao beanDao;

    public ColorBeanFragment() {
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        beanDao = GreenDaoManager.getmInstance(mContext).getDaoSession().getColorBeanDao();
...
//        ryList.setLayoutManager(new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL));
    ...
    protected void initData() {
        adapter = new BaseDataColorRyAdapter(mContext);
    ...
    private void searchData(){
        ...
        List<ColorBean> list = beanDao.queryBuilder().where(
                ColorBeanDao.Properties.FName.like("%"+edName.getText().toString()+"%")
    ...
    protected void initListener() {
        ...
        adapter.setOnItemClickListener(new RecyclerArrayAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                ColorBean thisNo = adapter.getAllData().get(position);
                Lg.e("点击",thisNo);
//                ShowNoteActivity.start(mContext,thisNo.id+"");
    ...
    private void delBuyBean(int position){
        ColorBean buyBean = adapter.getAllData().get(position);
...
//        }
    ...
    private void addAddrBean(){
        ...
        if (beanDao.queryBuilder().where(ColorBeanDao.Properties.FName.eq(edName.getText().toString())).count()>0){
            Toast.showText(mContext,"数据已存在，不能重复添加");
            return;
        }
        beanDao.insert(new ColorBean(edName.getText().toString(), CommonUtil.getTime(true), CommonUtil.getTimeLong(false)));
