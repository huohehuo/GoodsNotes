...
public class UnitBeanFragment extends BaseFragment {
    ...
    Button btnAdd;
    BaseDataUnitRyAdapter adapter;
    private FragmentActivity mContext;
    private UnitBeanDao beanDao;

    public UnitBeanFragment() {
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        beanDao = GreenDaoManager.getmInstance(mContext).getDaoSession().getUnitBeanDao();
...
//        ryList.setLayoutManager(new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL));
    ...
    protected void initData() {
        adapter = new BaseDataUnitRyAdapter(mContext);
    ...
    private void searchData(){
        ...
        List<UnitBean> list = beanDao.queryBuilder().where(
                UnitBeanDao.Properties.FName.like("%"+edName.getText().toString()+"%")
...
//列表点击事件
        adapter.setOnItemClickListener(new RecyclerArrayAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                UnitBean thisNo = adapter.getAllData().get(position);
                Lg.e("点击",thisNo);
//                ShowNoteActivity.start(mContext,thisNo.id+"");
    ...
    private void delBuyBean(int position){
        UnitBean buyBean = adapter.getAllData().get(position);
...
//        }
    ...
    private void addAddrBean(){
        ...
        if (beanDao.queryBuilder().where(UnitBeanDao.Properties.FName.eq(edName.getText().toString())).count()>0){
            Toast.showText(mContext,"数据已存在，不能重复添加");
            return;
        }
        beanDao.insert(new UnitBean(edName.getText().toString(), CommonUtil.getTime(true), CommonUtil.getTimeLong(false)));
