...
public class StuffBeanFragment extends BaseFragment {
    ...
    Button btnAdd;
    BaseDataStuffRyAdapter adapter;
    private FragmentActivity mContext;
    private StuffBeanDao beanDao;

    public StuffBeanFragment() {
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        beanDao = GreenDaoManager.getmInstance(mContext).getDaoSession().getStuffBeanDao();
...
//        ryList.setLayoutManager(new StaggeredGridLayoutManager(2, StaggeredGridLayoutManager.VERTICAL));
    ...
    protected void initData() {
        adapter = new BaseDataStuffRyAdapter(mContext);
    ...
    private void searchData(){
        ...
        List<StuffBean> list = beanDao.queryBuilder().where(
                StuffBeanDao.Properties.FName.like("%"+edName.getText().toString()+"%")
    ...
    protected void initListener() {
        ...
        adapter.setOnItemClickListener(new RecyclerArrayAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                StuffBean thisNo = adapter.getAllData().get(position);
                Lg.e("点击",thisNo);
//                ShowNoteActivity.start(mContext,thisNo.id+"");
    ...
    private void delBuyBean(int position){
        StuffBean buyBean = adapter.getAllData().get(position);
...
//        }
    ...
    private void addAddrBean(){
        ...
        if (beanDao.queryBuilder().where(StuffBeanDao.Properties.FName.eq(edName.getText().toString())).count()>0){
            Toast.showText(mContext,"数据已存在，不能重复添加");
            return;
        }
        beanDao.insert(new StuffBean(edName.getText().toString(), CommonUtil.getTime(true), CommonUtil.getTimeLong(false)));
